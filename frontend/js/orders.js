    ...
    ordersContent.innerHTML = orders.map(order => `
        <div class="order-card" data-status="${order.status}">
            <div class="order-header">
                <div>
                    <div class="order-id">${order.order_number || 'Order #' + order.id}</div>
            ...
            <div class="order-items">
                ${order.items.map(item => `
                    <div class="order-item">
                        <img src="${item.image_url || 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=300'}" 
                             alt="${item.name}" 
                             class="item-image"
...
function filterOrders(status) {
    ...
    const visibleOrders = Array.from(orderCards).filter(card => card.style.display !== 'none');
    const ordersContent = document.getElementById('ordersContent');
    ...
    if (cartCountElement) {
        ...
        if (user) {
            fetch(`${API_BASE_URL}/cart?user_id=${user.id}`)
                ...
                .finally(() => {
                    cartCountElement.style.display = parseInt(cartCountElement.textContent) > 0 ? 'inline-block' : 'none';
                });
        }
    }
}
